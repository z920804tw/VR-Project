...
public class TargetSetting : MonoBehaviour
{
    ...
    void Start()
    {
        isDead = false;
        isHit = false;
        levelGamemanager = GameObject.Find("GameManager").GetComponent<LevelGamemanager>();
        defaultTarget = GameObject.Find("MainTarget").transform;
    }
        ...
        switch (TargetType)                     //會去看這個物件的type是什麼，如果是殭屍跟骷髏就會有找敵人的動作
        {
            ...
            case TargetType.Skeleton:
                ...
                else if (TargetHP <= 0)
                {
                    ...
                    {
                        isDead = true;
                        levelGamemanager.AddScore(score, money);
